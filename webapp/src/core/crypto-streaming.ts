...
export const createStream = async (formValues) => {
    try {
        ...
        if (!window.fuel || !account) {
            openNotification("FUEL or Solana wallet is not currently connected.", `To utilize StreamPay features, please connect your wallet.`, MESSAGE_TYPE.INFO, () => { });
            return;
        }
...
export const initializeStream = async (_id: string, streamAddress: string, formValues: any) => {
    try {
        const { account } = store.getState().account;
        if (!window.fuel || !account) {
            openNotification("FUEL / Solana wallet is not currently connected.", `To utilize StreamPay features, please connect your wallet.`, MESSAGE_TYPE.INFO, () => { });
            return;
        }
